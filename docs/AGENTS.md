...
- **Superadmin** is `role: 'admin'` **plus** `User.is_superadmin = true` (never a new role value — every `authorize('admin')` gate keeps working, the flag only *adds* powers). `admin@delphic.in` is the seeded superadmin. Server-side, the extra powers are gated by `authorizeSuperadmin` / `loadSuperadminFlag` in `middleware/auth.js`, which **re-read the flag from the DB each request** — never trust an `is_superadmin` JWT claim (there isn't one). Frontend gating goes through `userCan(user, cap)` / `usePermissions(user).isSuperadmin`; superadmin-only caps: `overrideStage`, `editAnyUser`, `deleteRecords`. **Admin + BDA** share account-flow caps (`editAccount`, `editBroughtBy`, `unlockAccount`). Current superadmin-only surface: edit any user field incl. role / email / password / `is_superadmin` (`PATCH /users/:id`), edit locked accounts in place, `POST /accounts/:id/stage/override` / `POST /submissions/:id/stage/override` (free-form stage moves), and **record deletion** (see the soft-delete bullet below). Ordinary admins may also step a submission back one stage or reactivate `rejected`/`backout` via the normal stage endpoint (reason required). Deletion is soft-only — never a hard `DELETE`.
...
- **Superadmin stage override**: `POST /accounts/:id/stage/override` and `POST /submissions/:id/stage/override` (both `authorizeSuperadmin`) force any stage, ignoring the transition map / lock / gates; reason required, audited as `[override] …` in `stage_history`. Client boards route a disallowed drag to the matching override drawer (`AccountStageOverrideDrawer` / `SubmissionStageOverrideDrawer`) for a superadmin instead of the "Cannot move from X to Y" toast. There is no requirement-status override.
...
- **Reports dates** render via `formatReportDate` (`reportViews.js`) as `08 September 26`.
- **Joinings** (`GET /reports/joinings`, admin+sales) — joining = `Submission` `stage='closed'` with `actual_joining_date` in range; `{tables:[by_sourcer, by_interviewer(L1/L2 split), by_vendor]}`, grouped by joining month. **Time to submit** (`GET /reports/time-to-submit`, admin+sales) — one row per submission created in range; durations `submission.created_at → first internal_r1 scheduled_at → first submitted_to_client stage-history`, each as `{ms,label}` (`—` when unreached). Client: `joiningsSections()` / `timeToSubmitColumns()` in `reportViews.js`, both visible in `ALL_REPORTS`.
